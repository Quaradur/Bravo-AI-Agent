...
const GlobeIcon = ({ className }: { className: string }) => (
  <svg className={className} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M8 1.33333C9.84167 3.21667 10.6667 5.55 10.6667 8C10.6667 10.45 9.84167 12.7833 8 14.6667C6.15833 12.7833 5.33333 10.45 5.33333 8C5.33333 5.55 6.15833 3.21667 8 1.33333V1.33333Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)
...
const NotificationIcon = ({ className }: { className: string }) => (
  <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M13.73 21C13.5542 21.3031 13.3019 21.5547 12.9982 21.7295C12.6946 21.9044 12.3504 22.0001 12 22C11.6496 22.0001 11.3054 21.9044 11.0018 21.7295C10.6982 21.5547 10.4458 21.3031 10.27 21" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
)
...
export default function SiteHeader({ onPricingClick, onGetStartedClick }: SiteHeaderProps) {
  ...
  const languages = [
    "Deutsch", "English", "Español", "Español (Latinoamérica)", "Français", "Italiano",
    "Português (Brasil)", "Português (Portugal)", "Tiếng Việt", "Türkçe", "简体中文",
    "繁體中文", "日本語", "한국어", "العربية", "ไทย", "हिन्दी",
  ]
  ...
  return (
    <>
      <style>{`
        ...
        .font-system { font-family: -apple-system, BlinkMacSystemFont, "Segoe UI Variable Display", "Segoe UI", Helvetica, "Apple Color Emoji", Arial, sans-serif, "Segoe UI Emoji", "Segoe UI Symbol"; }
        .text-main { color: #34322D; }
        .text-subtle { color: #5E5E5B; }
        .btn-get-started { font-size: 14px; color: #FFFFFF; background-color: #1A1A19; }

        .baseline-reset { line-height: 1; }
        ...
        <div className="max-w-screen-2xl mx-auto px-12">
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-8">
              <a href="#" aria-label="Homepage" className="block shrink-0">
                {/* Logo leggermente più piccolo: ~32px */}
                <Logo className="block h-8 w-auto" />
              </a>

              <nav className="hidden md:flex items-center gap-6 text-sm font-medium baseline-reset">
            ...
            {/* Destra: Lingua, Notifiche, CTA */}
            <div className="flex items-center gap-6">
              <div className="relative hidden sm:block" ref={langMenuRef}>
                <button
                  onClick={() => setLangMenuOpen(!isLangMenuOpen)}
                  className="inline-flex h-10 items-center gap-1 rounded-lg px-2 text-sm text-main hover:bg-gray-100 transition-colors baseline-reset"
                >
                  <GlobeIcon className="w-4 h-4 block" />
                  <span className="baseline-reset">{selectedLang}</span>
                  <ChevronDownIcon className="w-4 h-4 block" />
                </button>

                {isLangMenuOpen && (
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-50">
                    <ul className="py-1 max-h-[360px] overflow-y-auto custom-scrollbar">
                      {languages.map((lang) => (
                        <li key={lang}>
                          ...
                          >
                            {lang}
                            {selectedLang === lang && <CheckIcon className="w-4 h-4" />}
                          </a>
                        </li>
                      ))}
                    </ul>
              ...
              <div className="tooltip-container">
                <button className="inline-flex h-10 w-10 items-center justify-center text-gray-900 hover:bg-gray-100 rounded-lg transition-colors">
                  <NotificationIcon className="w-5 h-5 block" />
              ...
              <button
                ...
                className="inline-flex h-10 items-center justify-center rounded-full px-4 hover:opacity-90 transition-opacity font-system btn-get-started baseline-reset"
              >
                Get Started
              </button>
            </div>
