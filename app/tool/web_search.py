...
class WebSearch(BaseTool):
    ...
    parameters: dict = {
        ...
        "properties": {
            ...
            "fetch_content": {"type": "boolean", "description": "Fetch content from result pages.", "default": False},
        },
        "required": ["query"],
    }
    ...
    async def execute(
        ...
        lang = lang or (getattr(config.search_config, "lang", "en") if config.search_config else "en")
        country = country or (getattr(config.search_config, "country", "us") if config.search_config else "us")
        ...
        for retry_count in range(max_retries + 1):
            ...
            if retry_count < max_retries:
                logger.warning(
                    f"All search engines failed. Waiting {retry_delay}s before retry {retry_count + 1}/{max_retries}..."
                )
        ...
        error_message = "All search engines failed to return results after multiple retries."
        if self.callback_handler:
            await self.callback_handler("summary", content=f"⚠️ Non sono riuscito a trovare risultati per '{query}': {error_message}")
    ...
    def _get_engine_order(self) -> List[str]:
        ...
        fallbacks = [e.lower() for e in config.search_config.fallback_engines] if config.search_config and hasattr(config.search_config, "fallback_engines") else []
